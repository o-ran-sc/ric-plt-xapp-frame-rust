...
pub trait SdlStorageApi {
    ...
    fn set(&self, namespace: &str, data: &DataMap) -> Result<(), SdlError>;
    ...
    fn set_if_not_exists(&self, namespace: &str, key: &str, value: &[u8]) -> Result<(), SdlError>;
    ...
    fn get(&self, namespace: &str, keys: &KeySet) -> Result<DataMap, SdlError>;
    ...
    fn delete(&self, namespace: &str, keys: &KeySet) -> Result<(), SdlError>;
    ...
    fn delete_if(&self, namespace: &str, key: &str, value: &[u8]) -> Result<bool, SdlError>;

    /// List keys from the SDL Storage that match the given pattern.
    ///
    /// This function can return an empty `KeySet`.
    fn list_keys(&self, namespace: &str, pattern: &str) -> Result<KeySet, SdlError>;

    /// Delete all Keys from the SDL Storage
    fn delete_all(&self, namespace: &str) -> Result<(), SdlError>;
}
